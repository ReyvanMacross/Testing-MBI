...
const root = path.join(PROJECT_ROOT, "app", "api", "diskop");
const files = await collect(root);
assert.ok(files.length > 0, "No Diskop API routes found.");
for (const file of files) {
  ...
  assert.match(source, /requireDiskopActor\s*\(/u, `${file} misses requireDiskopActor`);
}
console.log(`Diskop API guard audit: ${files.length} routes PASS`);
