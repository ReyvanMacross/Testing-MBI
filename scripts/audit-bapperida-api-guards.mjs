...
const files=await collect(path.join(PROJECT_ROOT,"app","api","bapperida"));
assert.ok(files.length>0,"No Bapperida API routes found.");
for(const file of files){const source=await readFile(file,"utf8");if(!/export\s+async\s+function\s+(POST|PATCH|PUT|DELETE)/u.test(source))continue;assert.match(source,/assertSameOrigin\s*\(/u,`${file} misses assertSameOrigin`);assert.match(source,/assertBodySize\s*\(/u,`${file} misses assertBodySize`);assert.match(source,/requireBapperidaActor\s*\(/u,`${file} misses requireBapperidaActor`);}
console.log(`Bapperida API guard audit: ${files.length} routes PASS`);
