...
const root = path.join(PROJECT_ROOT, "app", "api", "disbudpar");
const files = await collect(root);
assert.ok(files.length > 0, "No Disbudpar API routes found.");
for (const file of files) {
  ...
  assert.match(source, /requireDisbudparActor\s*\(/u, `${file} misses requireDisbudparActor`);
}
console.log(`Disbudpar API guard audit: ${files.length} routes PASS`);
