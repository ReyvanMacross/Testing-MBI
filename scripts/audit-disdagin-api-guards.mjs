...
const root = path.join(PROJECT_ROOT, "app", "api", "disdagin");
const files = await collect(root);
assert.ok(files.length > 0, "No Disdagin API routes found.");
for (const file of files) {
  ...
  assert.match(source, /requireDisdaginActor\s*\(/u, `${file} misses requireDisdaginActor`);
}
console.log(`Disdagin API guard audit: ${files.length} routes PASS`);
