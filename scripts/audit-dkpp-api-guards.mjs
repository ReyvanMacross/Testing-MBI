...
const root = path.join(PROJECT_ROOT, "app", "api", "dkpp");
const files = await collect(root);
assert.ok(files.length > 0, "No Dkpp API routes found.");
for (const file of files) {
  ...
  assert.match(source, /requireDkppActor\s*\(/u, `${file} misses requireDkppActor`);
}
console.log(`Dkpp API guard audit: ${files.length} routes PASS`);
