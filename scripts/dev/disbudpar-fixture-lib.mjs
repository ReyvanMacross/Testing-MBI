...
export const DEV_DISBUDPAR_CODES = {
  officers: ["DEV-PDB-01", "DEV-PDB-02"],
  programs: [
    "DEV-PRG-BUD-01",
    "DEV-PRG-BUD-QUOTA",
...
const stateFile = path.join(PROJECT_ROOT, "artifacts", "disbudpar", "fixture-state.json");
...
export async function cleanupDisbudparFixtures() {
  const db = await client();
  const saved = await savedState();
  const programResult = await db.from("master_program_layanan")
    .select("id").in("kode_program", DEV_DISBUDPAR_CODES.programs);
  ...
  const profileResult = await db.from("disbudpar_beneficiary_profiles").delete().eq("is_fixture", true);
  if (profileResult.error) throw profileResult.error;
  const officerResult = await db.from("disbudpar_pendamping").delete()
    .in("kode", DEV_DISBUDPAR_CODES.officers);
  if (officerResult.error) throw officerResult.error;

  const [remainingPrograms, remainingOfficers, remainingInterventions, remainingProfiles] = await Promise.all([
    db.from("master_program_layanan").select("id", { count: "exact", head: true })
      .in("kode_program", DEV_DISBUDPAR_CODES.programs),
    db.from("disbudpar_pendamping").select("id", { count: "exact", head: true })
      .in("kode", DEV_DISBUDPAR_CODES.officers),
    db.from("disbudpar_interventions").select("id", { count: "exact", head: true })
      .eq("is_fixture", true),
    db.from("disbudpar_beneficiary_profiles").select("id", { count: "exact", head: true })
      .eq("is_fixture", true),
  ]);
  for (const result of [remainingPrograms, remainingOfficers, remainingInterventions, remainingProfiles]) {
    if (result.error) throw result.error;
...
export async function seedDisbudparFixtures() {
  await cleanupDisbudparFixtures();
  try {
  const db = await client();
  const [dinsosActorResult, disbudparActorResult, opdResult, activeResult, candidateResult] = await Promise.all([
    db.from("user_profiles").select("id,opd_id").eq("email", "dinsos@bandung.go.id").single(),
    db.from("user_profiles").select("id,opd_id").eq("username", "admin.disbudpar").single(),
    db.from("master_opd").select("id,kode_opd").in("kode_opd", ["DINSOS", "DISBUDPAR"]),
  ...
  for (const result of [dinsosActorResult, disbudparActorResult, opdResult, activeResult, candidateResult]) {
    if (result.error) throw result.error;
  }
  const dinsosActor = dinsosActorResult.data;
  const disbudparActor = disbudparActorResult.data;
  ...
  if (candidates.length !== 6) throw new Error("Enam warga existing tanpa kasus aktif diperlukan untuk fixture DISBUDPAR.");

  const state = { officers: {}, programs: {}, referrals: {}, interventions: {}, actor: disbudparActor };

  const officerInsert = await db.from("disbudpar_pendamping").insert([
    { kode: "DEV-PDB-01", nama: "Pendamping Ekraf & Sanggar Seni Development", klaster: "Kriya & Seni Pertunjukan", lokasi: "Lokasi pengujian internal" },
    { kode: "DEV-PDB-02", nama: "Pendamping Kuota Development", klaster: "Musik dan Seni Tari", lokasi: "Lokasi pengujian internal" },
  ]).select("id,kode,nama");
  ...
  const programDefinitions = [
    { code: "DEV-PRG-BUD-01", name: "Pelatihan Kriya & Seni Pertunjukan Development", opd: "DISBUDPAR", path: "WIRAUSAHA", officer: "DEV-PDB-01", capacity: 20 },
    { code: "DEV-PRG-BUD-QUOTA", name: "Program Bantuan Kuota Development", opd: "DISBUDPAR", path: "WIRAUSAHA", officer: "DEV-PDB-02", capacity: 1 },
    { code: "DEV-PRG-DINSOS-WRONG", name: "Program Salah OPD Development", opd: "DINSOS", path: "PENGUATAN_DASAR", officer: null, capacity: 1 },
    { code: "DEV-PRG-PATH-WRONG", name: "Program Salah Jalur Development", opd: "DISBUDPAR", path: "PEKERJA", officer: null, capacity: 1 },
  ];
  const programs = {};
  for (const definition of programDefinitions) {
    ...
    if (definition.officer) {
      ...
      const detail = await db.from("disbudpar_program_details").insert({
        program_id: insert.data.id,
        category: "Kriya & Seni Pertunjukan",
        pendamping_id: officer.id,
        pendamping: officer.nama,
        location: "Lokasi pengujian internal",
        duration_value: 1,
        duration_unit: "BULAN",
        capacity: definition.capacity,
        description: "Program terkontrol untuk pengujian workflow DISBUDPAR.",
        facilitation: "Fasilitasi peralatan seni dan pendampingan ekraf internal",
      });
  ...
  const definitions = [
    { key: "primary", program: "DEV-PRG-BUD-01", target: "DISBUDPAR", path: "WIRAUSAHA" },
    { key: "completion", program: "DEV-PRG-BUD-01", target: "DISBUDPAR", path: "WIRAUSAHA" },
    { key: "quotaA", program: "DEV-PRG-BUD-QUOTA", target: "DISBUDPAR", path: "WIRAUSAHA" },
    { key: "quotaB", program: "DEV-PRG-BUD-QUOTA", target: "DISBUDPAR", path: "WIRAUSAHA" },
    { key: "report", program: "DEV-PRG-BUD-01", target: "DISBUDPAR", path: "WIRAUSAHA" },
  ...
  for (const [index, definition] of definitions.entries()) {
    ...
    const assessment = await db.from("dinsos_assessments").insert({
      ...
      assessment_date: today, observation: "Observasi fixture workflow Disbudpar.",
    ...
    const review = await db.from("dinsos_assessment_reviews").insert({
      ...
      target_opd_id: targetOpdId, reviewer_note: "Keputusan fixture workflow Disbudpar.",
    ...
    const pathResult = await db.from("penentuan_jalur").insert({
      ...
      route_reason: "Keputusan fixture workflow Disbudpar.", decision_source: "ASSESSMENT_REVIEW",
  ...
  return state;
    ...
    try {
      ...
      throw new AggregateError(
        [error, cleanupError],
        "Seed fixture Disbudpar gagal dan cleanup juga gagal.",
      );
    }
    throw error;
  }
