...
export async function cleanupDinsosFixtures() {
  ...
  const caseIds=[...new Set(Object.values(state??{}).map((item)=>typeof item==="string"?item:item?.caseId).filter(Boolean))];
  if(caseIds.length>3)throw new Error(`Fixture cleanup guard: found ${caseIds.length} exact case IDs.`);
  if(!caseIds.length){await rm(stateFile,{force:true});return 0;}
  const db=await client(); const {data,error}=await db.from("dinsos_cases").select("id").in("id",caseIds).eq("is_fixture",true); if(error)throw error;
...
export async function seedDinsosFixtures() {
  ...
  await persistState(state);return state;
}
