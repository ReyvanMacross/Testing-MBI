...
const stateFile = path.join(PROJECT_ROOT, "artifacts", "diskop", "fixture-state.json");
...
export async function cleanupDiskopFixtures() {
  const db = await client();
  const saved = await savedState();
  const programResult = await db.from("master_program_layanan")
    .select("id").in("kode_program", DEV_DISKOP_CODES.programs);
  ...
  const mentorResult = await db.from("diskop_pendamping").delete()
    .in("kode", DEV_DISKOP_CODES.mentors);
  if (mentorResult.error) throw mentorResult.error;

  const [remainingPrograms, remainingMentors, remainingInterventions] = await Promise.all([
    db.from("master_program_layanan").select("id", { count: "exact", head: true })
      .in("kode_program", DEV_DISKOP_CODES.programs),
    db.from("diskop_pendamping").select("id", { count: "exact", head: true })
      .in("kode", DEV_DISKOP_CODES.mentors),
    db.from("diskop_interventions").select("id", { count: "exact", head: true })
      .eq("is_fixture", true),
  ]);
  for (const result of [remainingPrograms, remainingMentors, remainingInterventions]) {
    if (result.error) throw result.error;
...
export async function seedDiskopFixtures() {
  await cleanupDiskopFixtures();
  try {
  const db = await client();
  const [dinsosActorResult, diskopActorResult, opdResult, activeResult, candidateResult] = await Promise.all([
    db.from("user_profiles").select("id,opd_id").eq("email", "dinsos@bandung.go.id").single(),
    db.from("user_profiles").select("id,opd_id").eq("username", "admin.diskop").single(),
  ...
  for (const result of [dinsosActorResult, diskopActorResult, opdResult, activeResult, candidateResult]) {
    if (result.error) throw result.error;
  }
  const dinsosActor = dinsosActorResult.data;
  const diskopActor = diskopActorResult.data;
  ...
  if (candidates.length !== 6) throw new Error("Enam warga existing tanpa kasus aktif diperlukan untuk fixture Diskop.");

  const state = { mentors: {}, programs: {}, referrals: {}, interventions: {}, actor: diskopActor };

  const mentorInsert = await db.from("diskop_pendamping").insert([
  ...
  const programDefinitions = [
    { code: "DEV-PRG-WIR-01", name: "Pendampingan Usaha Development", opd: "DISKOP", path: "WIRAUSAHA", mentor: "DEV-PLUT-01", capacity: 20 },
    { code: "DEV-PRG-WIR-QUOTA", name: "Pendampingan Kuota Development", opd: "DISKOP", path: "WIRAUSAHA", mentor: "DEV-PLUT-02", capacity: 1 },
    { code: "DEV-PRG-DINSOS-WRONG", name: "Program Salah OPD Development", opd: "DINSOS", path: "PENGUATAN_DASAR", mentor: null, capacity: 1 },
    { code: "DEV-PRG-PATH-WRONG", name: "Program Salah Jalur Development", opd: "DISKOP", path: "PEKERJA", mentor: null, capacity: 1 },
  ];
  const programs = {};
  for (const definition of programDefinitions) {
    ...
    if (definition.mentor) {
      ...
      const detail = await db.from("diskop_program_details").insert({
        ...
        description: "Program terkontrol untuk pengujian workflow Diskop.",
  ...
  const definitions = [
    { key: "primary", program: "DEV-PRG-WIR-01", target: "DISKOP", path: "WIRAUSAHA" },
    { key: "completion", program: "DEV-PRG-WIR-01", target: "DISKOP", path: "WIRAUSAHA" },
    { key: "quotaA", program: "DEV-PRG-WIR-QUOTA", target: "DISKOP", path: "WIRAUSAHA" },
    { key: "quotaB", program: "DEV-PRG-WIR-QUOTA", target: "DISKOP", path: "WIRAUSAHA" },
    { key: "report", program: "DEV-PRG-WIR-01", target: "DISKOP", path: "WIRAUSAHA" },
  ...
  for (const [index, definition] of definitions.entries()) {
    ...
    const assessment = await db.from("dinsos_assessments").insert({
      ...
      assessment_date: today, observation: "Observasi fixture workflow Diskop.",
    ...
    const review = await db.from("dinsos_assessment_reviews").insert({
      ...
      target_opd_id: targetOpdId, reviewer_note: "Keputusan fixture workflow Diskop.",
    ...
    const pathResult = await db.from("penentuan_jalur").insert({
      ...
      route_reason: "Keputusan fixture workflow Diskop.", decision_source: "ASSESSMENT_REVIEW",
  ...
  return state;
    ...
    try {
      ...
      throw new AggregateError(
        [error, cleanupError],
        "Seed fixture Diskop gagal dan cleanup juga gagal.",
      );
    }
    throw error;
  }
