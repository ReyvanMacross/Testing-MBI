...
const profileId = process.env.DISNAKER_ADMIN_PROFILE_ID;
const username = process.env.DISNAKER_ADMIN_USERNAME;
const password = process.env.DISNAKER_ADMIN_PASSWORD;
if (!profileId || !username || !password) {
  throw new Error("DISNAKER_ADMIN_PROFILE_ID, DISNAKER_ADMIN_USERNAME, dan DISNAKER_ADMIN_PASSWORD wajib dikonfigurasi.");
}
if (username !== "admin.disnaker" || password.length < 12) {
  throw new Error("Konfigurasi akun Disnaker tidak memenuhi kebijakan onboarding.");
}
...
if (profileError || !profile) throw new Error("Profile Admin Disnaker tidak ditemukan.");
const opd = Array.isArray(profile.master_opd) ? profile.master_opd[0] : profile.master_opd;
if (!profile.email || profile.role !== "INTERVENSI" || opd?.kode_opd !== "DISNAKER") {
  throw new Error("Profile yang dipilih bukan pengguna Disnaker yang sah.");
}
