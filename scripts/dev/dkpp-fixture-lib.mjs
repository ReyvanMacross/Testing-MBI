...
export const DEV_DKPP_CODES = {
  officers: ["DEV-PPL-01", "DEV-PPL-02"],
  programs: [
    "DEV-PRG-SAE-01",
    "DEV-PRG-SAE-QUOTA",
...
const stateFile = path.join(PROJECT_ROOT, "artifacts", "dkpp", "fixture-state.json");
...
export async function cleanupDkppFixtures() {
  const db = await client();
  const saved = await savedState();
  const programResult = await db.from("master_program_layanan")
    .select("id").in("kode_program", DEV_DKPP_CODES.programs);
  ...
  const profileResult = await db.from("dkpp_beneficiary_profiles").delete().eq("is_fixture", true);
  if (profileResult.error) throw profileResult.error;
  const officerResult = await db.from("dkpp_penyuluh").delete()
    .in("kode", DEV_DKPP_CODES.officers);
  ...
  const [remainingPrograms, remainingOfficers, remainingInterventions, remainingProfiles] = await Promise.all([
    db.from("master_program_layanan").select("id", { count: "exact", head: true })
      .in("kode_program", DEV_DKPP_CODES.programs),
    db.from("dkpp_penyuluh").select("id", { count: "exact", head: true })
      .in("kode", DEV_DKPP_CODES.officers),
    db.from("dkpp_interventions").select("id", { count: "exact", head: true })
      .eq("is_fixture", true),
    db.from("dkpp_beneficiary_profiles").select("id", { count: "exact", head: true })
      .eq("is_fixture", true),
  ]);
...
export async function seedDkppFixtures() {
  await cleanupDkppFixtures();
  try {
  const db = await client();
  const [dinsosActorResult, dkppActorResult, opdResult, activeResult, candidateResult] = await Promise.all([
    db.from("user_profiles").select("id,opd_id").eq("email", "dinsos@bandung.go.id").single(),
    db.from("user_profiles").select("id,opd_id").eq("username", "admin.dkpp").single(),
    db.from("master_opd").select("id,kode_opd").in("kode_opd", ["DINSOS", "DKPP"]),
  ...
  for (const result of [dinsosActorResult, dkppActorResult, opdResult, activeResult, candidateResult]) {
    if (result.error) throw result.error;
  }
  const dinsosActor = dinsosActorResult.data;
  const dkppActor = dkppActorResult.data;
  ...
  if (candidates.length !== 6) throw new Error("Enam warga existing tanpa kasus aktif diperlukan untuk fixture DKPP.");

  const state = { officers: {}, programs: {}, referrals: {}, interventions: {}, actor: dkppActor };

  const officerInsert = await db.from("dkpp_penyuluh").insert([
    { kode: "DEV-PPL-01", nama: "Penyuluh Buruan SAE Development", klaster: "Urban Farming", lokasi: "Lokasi pengujian internal" },
    { kode: "DEV-PPL-02", nama: "Penyuluh Kuota Development", klaster: "Bibit dan Ternak", lokasi: "Lokasi pengujian internal" },
  ]).select("id,kode,nama");
  ...
  const programDefinitions = [
    { code: "DEV-PRG-SAE-01", name: "Pelatihan Urban Farming Development", opd: "DKPP", path: "WIRAUSAHA", officer: "DEV-PPL-01", capacity: 20 },
    { code: "DEV-PRG-SAE-QUOTA", name: "Program Bantuan Kuota Development", opd: "DKPP", path: "WIRAUSAHA", officer: "DEV-PPL-02", capacity: 1 },
    { code: "DEV-PRG-DINSOS-WRONG", name: "Program Salah OPD Development", opd: "DINSOS", path: "PENGUATAN_DASAR", officer: null, capacity: 1 },
    { code: "DEV-PRG-PATH-WRONG", name: "Program Salah Jalur Development", opd: "DKPP", path: "PEKERJA", officer: null, capacity: 1 },
  ];
  const programs = {};
  for (const definition of programDefinitions) {
    ...
    if (definition.officer) {
      ...
      const detail = await db.from("dkpp_program_details").insert({
        program_id: insert.data.id,
        category: "Urban Farming",
        penyuluh_id: officer.id,
        penyuluh: officer.nama,
        location: "Lokasi pengujian internal",
        duration_value: 1,
        duration_unit: "BULAN",
        capacity: definition.capacity,
        description: "Program terkontrol untuk pengujian workflow DKPP.",
        facilitation: "Bantuan bibit dan pendampingan pangan internal",
      });
  ...
  const definitions = [
    { key: "primary", program: "DEV-PRG-SAE-01", target: "DKPP", path: "WIRAUSAHA" },
    { key: "completion", program: "DEV-PRG-SAE-01", target: "DKPP", path: "WIRAUSAHA" },
    { key: "quotaA", program: "DEV-PRG-SAE-QUOTA", target: "DKPP", path: "WIRAUSAHA" },
    { key: "quotaB", program: "DEV-PRG-SAE-QUOTA", target: "DKPP", path: "WIRAUSAHA" },
    { key: "report", program: "DEV-PRG-SAE-01", target: "DKPP", path: "WIRAUSAHA" },
  ...
  for (const [index, definition] of definitions.entries()) {
    ...
    const assessment = await db.from("dinsos_assessments").insert({
      ...
      assessment_date: today, observation: "Observasi fixture workflow Dkpp.",
    ...
    const review = await db.from("dinsos_assessment_reviews").insert({
      ...
      target_opd_id: targetOpdId, reviewer_note: "Keputusan fixture workflow Dkpp.",
    ...
    const pathResult = await db.from("penentuan_jalur").insert({
      ...
      route_reason: "Keputusan fixture workflow Dkpp.", decision_source: "ASSESSMENT_REVIEW",
  ...
  return state;
    ...
    try {
      ...
      throw new AggregateError(
        [error, cleanupError],
        "Seed fixture Dkpp gagal dan cleanup juga gagal.",
      );
    }
    throw error;
  }
