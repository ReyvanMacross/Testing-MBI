...
const root = path.join(PROJECT_ROOT, "app", "api", "dp3a");
const files = await collect(root);
assert.ok(files.length > 0, "Route API Dp3a tidak ditemukan.");
for (const file of files) {
  ...
  assert.match(source, /requireDp3aActor\s*\(/u, `${file} belum memakai requireDp3aActor`);
}
console.log(`Audit guard API Dp3a: ${files.length} route PASS`);
