...
import { Bell, Building2, ChartNoAxesCombined, ChevronDown, Handshake, LogOut, Menu, Search, UserRound, X } from "lucide-react";
import Link from "next/link";
import { usePathname, useRouter } from "next/navigation";
import type { ReactNode } from "react";
import { useEffect, useRef, useState } from "react";

import styles from "./cipta-bintar-shell.module.css";

const navigation = [
  { href: "/cipta-bintar", label: "Rujukan & Intervensi", icon: Handshake },
  { href: "/cipta-bintar/program", label: "Program Infrastruktur", icon: Building2 },
  { href: "/cipta-bintar/laporan", label: "Laporan Realisasi", icon: ChartNoAxesCombined },
] as const;

export function CiptaBintarShell({ children, namaLengkap }: { children: ReactNode; namaLengkap: string }) {
  ...
  return (
    <div className={styles.root}>
      ...
      <aside className={`${styles.sidebar} ${menuOpen ? styles.sidebarOpen : ""}`} aria-label="Navigasi Cipta Bintar">
        <div className={styles.sidebarBrand}>
          <span className={styles.sidebarLogo}><Building2 size={20} /></span>
          <div><strong>CIPTA BINTAR</strong><span>Kota Bandung</span></div>
          <button type="button" className={styles.sidebarClose} aria-label="Tutup navigasi" onClick={() => setMenuOpen(false)}><X size={20} /></button>
        </div>
        <nav>
          {navigation.map((item) => {
            const active = item.href === "/cipta-bintar" ? pathname === item.href : pathname.startsWith(item.href);
        ...
        <div className={styles.sidebarFooter}><strong>Sistem MBI</strong><span>Modul Infrastruktur &amp; Permukiman</span></div>
      </aside>
      <header className={styles.header}>
        ...
        <Link href="/cipta-bintar" className={styles.brand}>Platform MBI — Diskominfo Kota Bandung</Link>
        ...
        <div className={styles.account} ref={accountRef}>
          ...
          <button type="button" className={styles.accountTrigger} aria-haspopup="menu" aria-expanded={accountOpen} onClick={() => setAccountOpen((value) => !value)}>
            <span className={styles.avatar}>DB</span><span>Admin Dinas CiptaBintar</span><ChevronDown size={16} />
          </button>
          {accountOpen && (
