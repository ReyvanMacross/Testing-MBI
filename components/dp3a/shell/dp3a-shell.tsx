...
import { Bell, ChevronDown, ClipboardList, HeartHandshake, LogOut, Menu, Search, ShieldCheck, UserRound, X } from "lucide-react";
import Link from "next/link";
import { usePathname, useRouter } from "next/navigation";
import type { ReactNode } from "react";
import { useEffect, useRef, useState } from "react";
import styles from "./dp3a-shell.module.css";

const navigation = [
  { href: "/dp3a", label: "Rujukan & Penanganan", icon: ShieldCheck },
  { href: "/dp3a/program", label: "Program Layanan", icon: HeartHandshake },
  { href: "/dp3a/laporan", label: "Laporan Realisasi", icon: ClipboardList },
] as const;

export function Dp3aShell({ children, namaLengkap }: { children: ReactNode; namaLengkap: string }) {
  ...
  return <div className={styles.root}>
    ...
    <aside className={`${styles.sidebar} ${menuOpen ? styles.sidebarOpen : ""}`} aria-label="Navigasi DP3A">
      <div className={styles.sidebarBrand}><span className={styles.sidebarLogo}><ShieldCheck size={20} /></span><div><strong>DP3A</strong><span>Kota Bandung</span></div><button type="button" className={styles.sidebarClose} aria-label="Tutup navigasi" onClick={() => setMenuOpen(false)}><X size={20} /></button></div>
      ...
      <div className={styles.sidebarFooter}><strong>Sistem MBI</strong><span>Modul Perlindungan &amp; Pemberdayaan</span></div>
    </aside>
    <header className={styles.header}><button type="button" className={styles.menuButton} aria-label="Buka navigasi" onClick={() => setMenuOpen(true)}><Menu size={21} /></button><Link href="/dp3a" className={styles.brand}>Platform MBI — Diskominfo Kota Bandung</Link><label className={styles.search}><span className={styles.srOnly}>Cari</span><Search size={18} aria-hidden="true" /><input type="search" placeholder="Cari..." /></label><div className={styles.account} ref={accountRef}><button type="button" className={styles.bell} aria-label="Notifikasi"><Bell size={20} strokeWidth={1.7} /></button><button type="button" className={styles.accountTrigger} aria-haspopup="menu" aria-expanded={accountOpen} onClick={() => setAccountOpen((value) => !value)}><span className={styles.avatar}>DA</span><span>Admin DP3A</span><ChevronDown size={16} /></button>{accountOpen && <div className={styles.accountMenu} role="menu"><span role="menuitem"><UserRound size={15} /> {namaLengkap}</span><button type="button" role="menuitem" onClick={logout} disabled={busy}><LogOut size={15} />{busy ? "Keluar..." : "Keluar"}</button></div>}</div></header>
