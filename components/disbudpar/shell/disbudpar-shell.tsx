...
import { Bell, BookOpenCheck, ChartNoAxesCombined, ChevronDown, Handshake, LogOut, Menu, Palette, Search, UserRound, X } from "lucide-react";
import Link from "next/link";
import { usePathname, useRouter } from "next/navigation";
import type { ReactNode } from "react";
import { useEffect, useRef, useState } from "react";

import styles from "./disbudpar-shell.module.css";

const navigation = [
  { href: "/disbudpar", label: "Rujukan & Intervensi", icon: Handshake },
  { href: "/disbudpar/program", label: "Program Ekraf & Sanggar Seni", icon: BookOpenCheck },
  { href: "/disbudpar/laporan", label: "Laporan Pembinaan", icon: ChartNoAxesCombined },
] as const;

export function DisbudparShell({ children, namaLengkap }: { children: ReactNode; namaLengkap: string }) {
  ...
  return (
    <div className={styles.root}>
      ...
      <aside className={`${styles.sidebar} ${menuOpen ? styles.sidebarOpen : ""}`} aria-label="Navigasi Disbudpar">
        <div className={styles.sidebarBrand}>
          <span className={styles.sidebarLogo}><Palette size={20} /></span>
          <div><strong>DISBUDPAR</strong><span>Kota Bandung</span></div>
          <button type="button" className={styles.sidebarClose} aria-label="Tutup navigasi" onClick={() => setMenuOpen(false)}><X size={20} /></button>
        </div>
        <nav>
          {navigation.map((item) => {
            const active = item.href === "/disbudpar" ? pathname === item.href : pathname.startsWith(item.href);
        ...
        <div className={styles.sidebarFooter}><strong>Sistem MBI</strong><span>Modul Budpar &amp; Ekraf</span></div>
      </aside>
      <header className={styles.header}>
        ...
        <Link href="/disbudpar" className={styles.brand}>Platform MBI — Diskominfo Kota Bandung</Link>
        ...
        <div className={styles.account} ref={accountRef}>
          ...
          <button type="button" className={styles.accountTrigger} aria-haspopup="menu" aria-expanded={accountOpen} onClick={() => setAccountOpen((value) => !value)}>
            <span className={styles.avatar}>DP</span><span>Admin DISBUDPAR</span><ChevronDown size={16} />
          </button>
          {accountOpen && (
