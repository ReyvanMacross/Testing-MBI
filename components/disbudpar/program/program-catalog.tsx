...
import { CalendarDays, ChevronLeft, ChevronRight, Download, Eye, Palette, Printer, UserRoundCheck, UsersRound } from "lucide-react";
...
import type { DisbudparCreativeResultStatus, DisbudparOfficer, DisbudparParticipantStatus, DisbudparProgram, DisbudparReferral } from "@/lib/disbudpar/data";
...
export function ProgramCatalog({
  ...
  initialPrograms: DisbudparProgram[];
  referrals: DisbudparReferral[];
  officers: DisbudparOfficer[];
}) {
  const [programs, setPrograms] = useState(initialPrograms);
  const [referrals, setReferrals] = useState(initialReferrals);
  const [modal, setModal] = useState<Modal>(null);
  const [selectedProgram, setSelectedProgram] = useState<DisbudparProgram | null>(null);
  const [selectedParticipant, setSelectedParticipant] = useState<DisbudparReferral | null>(null);
  ...
  function openProgram(program: DisbudparProgram, next: Modal) {
  ...
  function openParticipant(participant: DisbudparReferral, next: Modal) {
  ...
  async function addProgram(event: React.FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch("/api/disbudpar/programs", {
      method: "POST",
    ...
    const officer = officers.find((item) => item.id === form.get("pendampingId"));
    setPrograms((items) => [{
      id: body.programId,
      code: String(form.get("code")),
      name: String(form.get("name")),
      category: String(form.get("category")),
      pendampingId: String(form.get("pendampingId")),
  ...
  async function allocateParticipant(event: React.FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch(`/api/disbudpar/referrals/${candidate.id}/start`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({
        programId: selectedProgram.id,
        pendampingId: selectedProgram.pendampingId,
        groupName: form.get("groupName"),
        creativeSubsector: selectedProgram.category,
    ...
    setReferrals((items) => items.map((item) => item.id === candidate.id ? {
      ...
      timeline: [...item.timeline, {
        ...
        type: "STARTED",
        date: "Hari ini",
        note: "Penerima dialokasikan ke program ekraf dan sanggar seni.",
        progress: 0,
      }],
    } : item));
  ...
  async function updateParticipant(event: React.FormEvent<HTMLFormElement>) {
    ...
    const endpoint = isCompletion
      ? `/api/disbudpar/interventions/${selectedParticipant.interventionId}/complete`
      : `/api/disbudpar/interventions/${selectedParticipant.interventionId}/progress`;
    ...
    const creativeResultStatus = isCompletion
      ? "AKTIF_TAMPIL_PRODUKSI_RUTIN"
      : String(form.get("creativeResultStatus")) as DisbudparCreativeResultStatus;
    ...
    setReferrals((items) => items.map((item) => item.id === selectedParticipant.id ? {
      ...
      evaluation,
      achievementValue: isCompletion ? Number(form.get("achievementValue")) : item.achievementValue,
      timeline: [...item.timeline, {
        id: crypto.randomUUID(),
  ...
  return <section aria-labelledby="program-title">
    <div className={styles.pageHeading}>
      <div>
        <h1 id="program-title">Katalog Ekraf &amp; Sanggar Seni</h1>
        <p>Daftar program pembinaan, fasilitasi peralatan seni, dan alokasi kuota pendampingan sanggar MBI.</p>
      </div>
    ...
    <div className={styles.summaryGrid}>
      ...
      <article className={styles.summaryCard}><span className={styles.summaryIcon}><UsersRound size={27} /></span><div><p>Total Kuota Teralokasi</p><strong>{programs.reduce((sum, program) => sum + program.capacity, 0)} <small>Warga/Sanggar</small></strong></div></article>
      <article className={styles.summaryCard}><span className={styles.summaryIcon}><UserRoundCheck size={27} /></span><div><p>Sanggar / Kelompok Mandiri</p><strong>{referrals.filter((item) => item.status === "MANDIRI_SELESAI").length} <small>Komunitas</small></strong></div></article>
    </div>

    <section className={styles.card} aria-label="Katalog ekraf dan sanggar seni">
      ...
      {!filtered.length && <p className={styles.emptyState}>Belum ada program Disbudpar yang sesuai filter.</p>}
    ...
    {modal === "add" && <Dialog title="Tambah Program / Bantuan Baru" close={close} footer={<><button className={styles.secondary} onClick={close}>Batal</button><button type="submit" form="add-program" className={styles.primary}>Simpan &amp; Terbitkan Program</button></>}>
      <form id="add-program" className={styles.form} onSubmit={addProgram}>
        <div className={styles.twoColumns}><label>Kode Program<input name="code" defaultValue={`PRG-BUD-${String(programs.length + 1).padStart(2, "0")}`} pattern="PRG-BUD-[0-9]{2}" required /></label><label>Nama Program / Bantuan<input name="name" placeholder="Workshop Inkubasi Sanggar & Galeri Seni 2026" minLength={5} required /></label></div>
        <label>Kategori Program<select name="category" defaultValue="Kriya, Seni Rupa, & Pertunjukan" required><option>Kriya, Seni Rupa, &amp; Pertunjukan</option><option>Seni Musik &amp; Seni Tari</option><option>Kuliner &amp; Desain Grafis</option><option>Film, Fotografi, &amp; Animasi</option></select></label>
        <label>Pendamping DISBUDPAR<select name="pendampingId" required><option value="">Pilih pendamping</option>{officers.map((officer) => <option key={officer.id} value={officer.id}>{officer.name} ({officer.cluster})</option>)}</select></label>
        ...
        <label>Deskripsi Fasilitas &amp; Jenis Bantuan<textarea name="description" placeholder="Jelaskan peralatan seni, modul pelatihan, ruang pamer, dan pendampingan." minLength={10} required /></label>
    ...
    {modal === "allocate" && selectedProgram && <Dialog title="Alokasikan Penerima Program" close={() => setModal("manage")} footer={<><button className={styles.secondary} onClick={() => setModal("manage")}>Batal</button><button className={styles.primary} type="submit" form="allocate-form">Simpan Alokasi</button></>}>
      <form id="allocate-form" className={styles.form} onSubmit={allocateParticipant}>
        <div className={styles.identity}><div className={styles.twoColumns}><div><small>Agenda</small><h3>{selectedProgram.name}</h3></div><div><small>Sisa Kuota Program</small><h3>{Math.max(0, selectedProgram.capacity - selectedProgram.filled)} Sanggar/Kelompok</h3></div></div></div>
        ...
        <label>Nama Kelompok / Sanggar<input name="groupName" placeholder="Sanggar Kriya Bambu Sukajadi" minLength={3} required /></label>
        <label>Lokasi Sanggar / Galeri<input name="venueLocation" placeholder="Galeri Kriya Sukajadi RT 03" minLength={3} required /></label>
        <label>Fasilitas &amp; Jenis Bantuan<input name="aidPackage" defaultValue={selectedProgram.facilitation} minLength={3} required /></label>
        <label>Catatan Penempatan Program<textarea name="actionPlan" defaultValue="Alokasi fasilitas seni dan pendampingan program ekraf serta sanggar seni." minLength={10} required /></label>
        {notice && <p role="status">{notice}</p>}
      </form>
    </Dialog>}
    ...
    {modal === "participant" && selectedParticipant && selectedProgram && <Dialog title={`Update Status Keikutsertaan — ${selectedParticipant.name}`} close={() => setModal("manage")} footer={<><button className={styles.secondary} onClick={() => setModal("manage")}>Batal</button><button className={styles.primary} type="submit" form="participant-form">{completionSelected ? "Simpan & Selesaikan Pendampingan" : "Simpan Progress"}</button></>}>
      <form id="participant-form" className={styles.form} onSubmit={updateParticipant}>
        ...
        {!completionSelected && <div className={styles.twoColumns}><label>Progress Pendampingan (%)<input name="progressPercent" type="number" min="0" max="99" defaultValue={selectedParticipant.progress ?? 0} required /></label><label>Status Hasil Ekraf &amp; Seni<select name="creativeResultStatus" defaultValue={selectedParticipant.creativeResultStatus ?? "BELUM_AKTIF"} required><option value="BELUM_AKTIF">Tahap Verifikasi</option><option value="AKTIF_TERBATAS">Penerima Bantuan / Pembinaan</option><option value="AKTIF_TAMPIL_PRODUKSI_RUTIN">Aktif Tampil &amp; Produksi Rutin</option></select></label></div>}
        {completionSelected && <><div className={styles.twoColumns}><label>Lokasi Sanggar / Galeri<input value={selectedParticipant.venueLocation ?? "Belum ditentukan"} readOnly /></label><label>Capaian Omzet / Nilai Tampil (Rp)<input name="achievementValue" type="number" min="0" max="1000000000000" required /></label></div><label>Tanggal Selesai Pendampingan<input name="completionDate" type="date" defaultValue={today} required /></label></>}
        <label>Catatan Progress DISBUDPAR<textarea name="evaluation" defaultValue={selectedParticipant.evaluation ?? ""} minLength={10} required /></label>
        {notice && <p role="status">{notice}</p>}
      </form>
    </Dialog>}
    ...
    {modal === "participantDetail" && selectedParticipant && <Dialog title={`Detail Penerima — ${selectedParticipant.code}`} close={() => setModal("manage")} footer={<><button className={styles.secondary} onClick={() => window.print()}><Printer size={16} /> Unduh Rekap Program</button><button className={styles.secondary} onClick={() => setModal("manage")}>Tutup</button></>}>
      <div className={styles.detailStack}><div className={styles.identity}><small>Nama Warga</small><h3>{selectedParticipant.name} ({selectedParticipant.maskedNik})</h3><div className={styles.twoColumns}><div><small>Lokasi Sanggar / Galeri</small><strong>{selectedParticipant.venueLocation ?? "—"}</strong></div><div><small>Omzet / Nilai Tampil</small><strong>{selectedParticipant.achievementValue === null ? "—" : rupiah.format(selectedParticipant.achievementValue)}</strong></div></div><span className={`${styles.badge} ${styles.badgeGreen}`}>Mandiri / Selesai</span></div><section><h2>Riwayat Pendampingan</h2><div className={styles.timeline}>{selectedParticipant.timeline.map((entry) => <div key={entry.id}><span className={styles.muted}>{entry.date}</span><p>{entry.note}{entry.progress === null ? "" : ` — Progress ${entry.progress}%`}</p></div>)}</div></section></div>
    </Dialog>}
    ...
    {modal === "programDetail" && selectedProgram && <Drawer title={`Detail Program — ${selectedProgram.code}`} close={close} footer={<><button className={styles.secondary} onClick={() => window.print()}><Download size={16} /> Unduh Rekap Program</button><button className={styles.secondary} onClick={close}>Tutup</button></>}>
      <div className={styles.detailStack}><div className={styles.identity}><small>Nama Program</small><h3>{selectedProgram.name}</h3><p>{selectedProgram.category} | {selectedProgram.duration}</p><span className={`${styles.badge} ${selectedProgram.status === "PENUH" ? styles.badgeAmber : styles.badgeGreen}`}>{selectedProgram.status} ({selectedProgram.filled}/{selectedProgram.capacity} SANGGAR TERISI)</span></div><div className={styles.twoColumns}><div className={styles.detailBox}><small>Fasilitas Utama</small><strong>{selectedProgram.facilitation}</strong></div><div className={styles.detailBox}><small>Penerima Manfaat</small><strong>{selectedProgram.capacity} Sanggar / Kelompok Seni</strong></div></div><section><h2>Riwayat Tahapan Program</h2>{referrals.filter((item) => item.programId === selectedProgram.id).map((item) => <div className={styles.detailBox} key={item.id}><Palette size={20} /><strong>{item.name}</strong><span className={styles.muted}>{item.maskedNik}<br />{item.venueLocation ?? item.kelurahan}</span></div>)}</section></div>
    </Drawer>}
  </section>;
}
