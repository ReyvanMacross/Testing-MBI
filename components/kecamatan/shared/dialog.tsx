...
export function Dialog({ title, children, footer, close, wide = false }: { title: string; children: ReactNode; footer?: ReactNode; close: () => void; wide?: boolean }) {
  ...
  return <div className={styles.overlay} role="presentation" onMouseDown={(event) => { if (event.currentTarget === event.target) close(); }}><div ref={ref} className={`${styles.dialog} ${wide ? styles.dialogWide : ""}`} role="dialog" aria-modal="true" aria-labelledby="kc-dialog-title" tabIndex={-1}><header><h2 id="kc-dialog-title">{title}</h2><button type="button" onClick={close} aria-label="Tutup dialog"><X size={23} /></button></header><div className={styles.dialogBody}>{children}</div>{footer && <footer>{footer}</footer>}</div></div>;
}

export function Drawer({ title, children, footer, close }: { title: string; children: ReactNode; footer?: ReactNode; close: () => void }) {
  const ref = useFokusDialog<HTMLElement>();
  useEffect(() => { const onKey = (event: KeyboardEvent) => { if (event.key === "Escape") close(); }; document.addEventListener("keydown", onKey); return () => document.removeEventListener("keydown", onKey); }, [close]);
  return <div className={styles.drawerOverlay} role="presentation" onMouseDown={(event) => { if (event.currentTarget === event.target) close(); }}><aside ref={ref} className={styles.drawer} role="dialog" aria-modal="true" aria-labelledby="kc-drawer-title" tabIndex={-1}><header><h2 id="kc-drawer-title">{title}</h2><button type="button" onClick={close} aria-label="Tutup panel"><X size={23} /></button></header><div className={styles.drawerBody}>{children}</div>{footer && <footer>{footer}</footer>}</aside></div>;
}
