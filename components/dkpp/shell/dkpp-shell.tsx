...
import { Bell, BookOpenCheck, ChartNoAxesCombined, ChevronDown, Handshake, Leaf, LogOut, Menu, Search, UserRound, X } from "lucide-react";
import Link from "next/link";
import { usePathname, useRouter } from "next/navigation";
import type { ReactNode } from "react";
import { useEffect, useRef, useState } from "react";

import styles from "./dkpp-shell.module.css";

const navigation = [
  { href: "/dkpp", label: "Rujukan & Intervensi", icon: Handshake },
  { href: "/dkpp/program", label: "Program Buruan SAE", icon: BookOpenCheck },
  { href: "/dkpp/laporan", label: "Laporan Ketahanan Pangan", icon: ChartNoAxesCombined },
] as const;

export function DkppShell({ children, namaLengkap }: { children: ReactNode; namaLengkap: string }) {
  ...
  return (
    <div className={styles.root}>
      ...
      <aside className={`${styles.sidebar} ${menuOpen ? styles.sidebarOpen : ""}`} aria-label="Navigasi Dkpp">
        <div className={styles.sidebarBrand}>
          <span className={styles.sidebarLogo}><Leaf size={20} /></span>
          <div><strong>DKPP</strong><span>Kota Bandung</span></div>
          <button type="button" className={styles.sidebarClose} aria-label="Tutup navigasi" onClick={() => setMenuOpen(false)}><X size={20} /></button>
        </div>
        <nav>
          {navigation.map((item) => {
            const active = item.href === "/dkpp" ? pathname === item.href : pathname.startsWith(item.href);
        ...
        <div className={styles.sidebarFooter}><strong>Sistem MBI</strong><span>Modul Ketahanan Pangan</span></div>
      </aside>
      <header className={styles.header}>
        ...
        <Link href="/dkpp" className={styles.brand}>Platform MBI — Diskominfo Kota Bandung</Link>
        ...
        <div className={styles.account} ref={accountRef}>
          ...
          <button type="button" className={styles.accountTrigger} aria-haspopup="menu" aria-expanded={accountOpen} onClick={() => setAccountOpen((value) => !value)}>
            <span className={styles.avatar}>DP</span><span>Admin DKPP</span><ChevronDown size={16} />
          </button>
          {accountOpen && (
