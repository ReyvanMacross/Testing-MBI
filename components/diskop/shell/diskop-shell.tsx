...
import styles from "./diskop-shell.module.css";

const navigation = [
  { href: "/diskop", label: "Rujukan & Intervensi", icon: Handshake },
  { href: "/diskop/program", label: "Program Pendampingan", icon: BookOpenCheck },
  { href: "/diskop/laporan", label: "Laporan Omzet", icon: ChartNoAxesCombined },
] as const;

export function DiskopShell({ children, namaLengkap }: { children: ReactNode; namaLengkap: string }) {
  ...
  return (
    <div className={styles.root}>
      ...
      <aside className={`${styles.sidebar} ${menuOpen ? styles.sidebarOpen : ""}`} aria-label="Navigasi Diskop UKM">
        <div className={styles.sidebarBrand}>
          <span className={styles.sidebarLogo}><Store size={20} /></span>
          <div><strong>Diskop UKM</strong><span>Kota Bandung</span></div>
          <button type="button" className={styles.sidebarClose} aria-label="Tutup navigasi" onClick={() => setMenuOpen(false)}><X size={20} /></button>
        </div>
        <nav>
          {navigation.map((item) => {
            const active = item.href === "/diskop" ? pathname === item.href : pathname.startsWith(item.href);
        ...
        <div className={styles.sidebarFooter}><strong>Sistem MBI</strong><span>Modul Pemberdayaan UMKM</span></div>
      </aside>
      <header className={styles.header}>
        ...
        <Link href="/diskop" className={styles.brand}>Platform MBI — Diskominfo Kota Bandung</Link>
        ...
        <div className={styles.account} ref={accountRef}>
          ...
          <button type="button" className={styles.accountTrigger} aria-haspopup="menu" aria-expanded={accountOpen} onClick={() => setAccountOpen((value) => !value)}>
            <span className={styles.avatar}>DU</span><span>Admin DiskopUKM</span><ChevronDown size={16} />
          </button>
          {accountOpen && (
