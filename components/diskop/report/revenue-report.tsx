...
import { Drawer } from "@/components/diskop/shared/dialog";
import styles from "@/components/diskop/shared/diskop-ui.module.css";
import type { BusinessReport } from "@/lib/diskop/data";
...
export function RevenueReport({ businesses, summary }: {
  ...
  return <section aria-labelledby="revenue-report-title">
    <div className={styles.pageHeading}>
      <div><h1 id="revenue-report-title">Laporan Omzet &amp; Kemandirian UMKM MBI</h1><p>Rekapitulasi pencapaian omzet bulanan dan legalitas warga MBI yang telah berwirausaha mandiri.</p></div>
    ...
    <div className={styles.summaryGrid}>
      <article className={styles.summaryCard}><span className={styles.summaryIcon}><Store size={27} /></span><div><p>Total UMKM Mandiri</p><strong>{summary.independent} <small>Usaha</small></strong></div></article>
      <article className={styles.summaryCard}><span className={styles.summaryIcon}><WalletCards size={27} /></span><div><p>Total Omzet Bulanan MBI</p><strong>{rupiah.format(summary.totalRevenue)}</strong></div></article>
      <article className={styles.summaryCard}><span className={styles.summaryIcon}><TrendingUp size={27} /></span><div><p>Rata-rata Omzet / UMKM</p><strong>{rupiah.format(summary.averageRevenue)} <small>/bln</small></strong></div></article>
    </div>
    ...
    <section className={styles.card} aria-label="Laporan usaha mandiri">
      <div className={styles.filters}>
        ...
        <div className={styles.filterControls}><select value={category} onChange={(event) => setCategory(event.target.value)} aria-label="Kategori usaha"><option value="">Semua Kategori Usaha</option>{categories.map((item) => <option key={item}>{item}</option>)}</select><select aria-label="Status kemandirian" defaultValue="MANDIRI"><option value="MANDIRI">Status Kemandirian (Mandiri)</option></select></div>
      </div>
      <div className={styles.desktopTable}><table><thead><tr><th>ID Referral &amp; Nama Warga</th><th>Jenis Usaha &amp; NIB</th><th>Kategori Usaha</th><th>Capaian Omzet / Bln</th><th>Status Kemandirian</th><th>Aksi</th></tr></thead><tbody>{filtered.map((business) => <tr key={business.id}>
        ...
        <td><span className={`${styles.badge} ${styles.badgeGreen}`}>Mandiri</span></td><td><button className={styles.action} onClick={() => setSelected(business)}><Eye size={16} /> Detail</button></td>
      </tr>)}</tbody></table></div>
      <div className={styles.mobileCards}>{filtered.map((business) => <article key={business.id}><header><div><h3>{business.name}</h3><span className={styles.muted}>{business.referralCode}<br />{business.maskedNik}</span></div><span className={`${styles.badge} ${styles.badgeGreen}`}>Mandiri</span></header><dl><div><dt>Usaha / NIB</dt><dd>{business.businessName}<br />{business.nib}</dd></div><div><dt>Omzet / Bulan</dt><dd>{rupiah.format(business.monthlyRevenue)}</dd></div></dl><button className={styles.action} onClick={() => setSelected(business)}><Eye size={16} /> Detail</button></article>)}</div>
    ...
    {selected && <Drawer title={`Detail Laporan Omzet & Kemandirian — ${selected.referralCode}`} close={() => setSelected(null)} footer={<><button className={styles.secondary} onClick={() => window.print()}><Download size={16} /> Unduh Laporan Performa</button><button className={styles.secondary} onClick={() => setSelected(null)}>Tutup</button></>}>
      <div className={styles.detailStack}>
        <div className={styles.identity}><h3>{selected.name}</h3><p>NIK: {selected.maskedNik}<br />NIB: {selected.nib}<br />Desil: {selected.desil || "—"} | Kelurahan: {selected.kelurahan}<br />Jenis Usaha: {selected.businessName}</p><span className={`${styles.badge} ${styles.badgeGreen}`}>Mandiri</span></div>
