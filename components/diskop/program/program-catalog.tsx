...
import { ChevronLeft, ChevronRight, Download, Eye, GraduationCap, Printer, Store, UserRoundCheck, UsersRound } from "lucide-react";
...
import type { DiskopLegalStatus, DiskopMentor, DiskopParticipantStatus, DiskopProgram, DiskopReferral } from "@/lib/diskop/data";
...
export function ProgramCatalog({ initialPrograms, referrals: initialReferrals, mentors }: {
  initialPrograms: DiskopProgram[];
  referrals: DiskopReferral[];
  mentors: DiskopMentor[];
}) {
  const [programs, setPrograms] = useState(initialPrograms);
  const [referrals, setReferrals] = useState(initialReferrals);
  const [modal, setModal] = useState<Modal>(null);
  const [selectedProgram, setSelectedProgram] = useState<DiskopProgram | null>(null);
  const [selectedParticipant, setSelectedParticipant] = useState<DiskopReferral | null>(null);
  ...
  function openProgram(program: DiskopProgram, next: Modal) {
    setSelectedProgram(program); setSelectedParticipant(null); setNotice(""); setModal(next);
  }
  function openParticipant(participant: DiskopReferral, next: Modal) {
  ...
  async function addProgram(event: React.FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch("/api/diskop/programs", {
  ...
  async function updateParticipant(event: React.FormEvent<HTMLFormElement>) {
    ...
    const endpoint = isCompletion ? `/api/diskop/interventions/${selectedParticipant.interventionId}/complete` : `/api/diskop/interventions/${selectedParticipant.interventionId}/progress`;
    ...
    const legalStatus = isCompletion ? "LEGAL" : String(form.get("legalStatus")) as DiskopLegalStatus;
  ...
  return <section aria-labelledby="program-title">
    <div className={styles.pageHeading}><div><h1 id="program-title">Katalog Program Pendampingan Wirausaha</h1><p>Pengelolaan modul inkubasi bisnis, alokasi bantuan stimulan, dan kemitraan konsultan PLUT-KUMKM.</p></div><button className={styles.primary} onClick={() => setModal("add")} disabled={!mentors.length}>+ Tambah Program</button></div>
    <div className={styles.summaryGrid}>
      ...
      <article className={styles.summaryCard}><span className={styles.summaryIcon}><UsersRound size={27} /></span><div><p>Total Kuota Usaha</p><strong>{programs.reduce((sum, program) => sum + program.capacity, 0)} <small>Usaha</small></strong></div></article>
    ...
    <section className={styles.card} aria-label="Katalog program pendampingan">
      ...
      <div className={styles.desktopTable}><table><thead><tr><th>Kode &amp; Nama Program</th><th>Konsultan / Pendamping</th><th>Durasi</th><th>Kuota Terisi</th><th>Status</th><th>Aksi</th></tr></thead><tbody>{filtered.map((program) => { const percent = program.capacity ? Math.round(Math.min(100, program.filled / program.capacity * 100) * 100) / 100 : 0; return <tr key={program.id}><td><strong>{program.name}</strong><span className={styles.muted}>{program.code}</span></td><td><strong>{program.consultant}</strong><span className={styles.muted}>{program.cluster}</span></td><td>{program.duration}</td><td><span className={styles.progressTrack}><span style={{ width: `${percent}%`, background: program.status === "PENUH" ? "#9b7600" : percent > 80 ? "#236b39" : "#1457ad" }} /></span>{program.filled} / {program.capacity}</td><td><span className={`${styles.badge} ${program.status === "PENUH" ? styles.badgeAmber : program.status === "AKTIF" ? styles.badgeGreen : ""}`}>{program.status}</span></td><td>{program.status === "AKTIF" ? <button className={`${styles.action} ${styles.actionPrimary}`} onClick={() => openProgram(program, "manage")}>Kelola Kelas</button> : <button className={styles.action} onClick={() => openProgram(program, "programDetail")}><Eye size={16} /> Detail</button>}</td></tr>; })}</tbody></table></div>
      ...
      {!filtered.length && <p className={styles.emptyState}>Belum ada program pendampingan yang sesuai filter.</p>}
