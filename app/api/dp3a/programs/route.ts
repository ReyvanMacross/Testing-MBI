...
import { parseProgramInput } from "@/lib/dp3a/input";
...
export async function POST(request: Request) {
  try {
    ...
    const { data, error } = await admin.rpc("dp3a_create_program", {
      p_actor_id: actor.profileId, p_actor_opd_id: actor.opdId, p_code: input.code,
      p_name: input.name, p_category: input.category, p_service_type: input.serviceType, p_unit_id: input.unitId,
      p_duration_value: input.duration, p_duration_unit: input.durationUnit,
      p_execution_date: input.executionDate, p_budget_per_beneficiary: input.budgetPerBeneficiary,
    ...
    if (error?.message.includes("DP3A_ACTOR_REQUIRED")) throw new ApiError("Akses ditolak.", 403);
    if (error) throw new ApiError("Program tidak dapat disimpan.", 400);
    await writeActivityLog({ userId: actor.profileId, namaPengguna: actor.namaLengkap, rolePengguna: actor.role, aktivitas: "Menambahkan program layanan DP3A", modul: "DP3A", metadata: { programId: data.programId } });
    return NextResponse.json({ ok: true, ...data }, { status: 201 });
  } catch (error) { return apiErrorResponse(error, "DP3A program creation failed", "Program tidak dapat disimpan."); }
}
