...
import { assertReferralId, parseStartInterventionInput } from "@/lib/disdagin/input";
...
function mapError(error: { message?: string } | null) {
  ...
  if (message.includes("TARGET_OPD_REQUIRED") || message.includes("DISDAGIN_ACTOR_REQUIRED")) return new ApiError("Akses referral ditolak.", 403);
  if (message.includes("INVALID_PROGRAM")) return new ApiError("Program tidak aktif atau tidak sesuai Disdagin.", 400);
  ...
  if (message.includes("NIB_ALREADY_EXISTS") || message.includes("duplicate key")) return new ApiError("NIB sudah digunakan oleh usaha lain.", 409);
...
export async function POST(request: Request, { params }: { params: Promise<{ referralId: string }> }) {
  try {
    ...
    const { data, error } = await admin.rpc("disdagin_start_intervention", {
    ...
    await writeActivityLog({ userId: actor.profileId, namaPengguna: actor.namaLengkap, rolePengguna: actor.role, aktivitas: "Memulai fasilitasi akses pasar", modul: "Disdagin", metadata: { referralId, programId: input.programId } });
    return NextResponse.json({ ok: true, ...data });
  } catch (error) { return apiErrorResponse(error, "Disdagin intervention start failed", "Pendampingan tidak dapat dimulai."); }
}
