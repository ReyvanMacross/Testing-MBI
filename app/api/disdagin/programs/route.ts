...
import { parseProgramInput } from "@/lib/disdagin/input";
...
export async function POST(request: Request) {
  try {
    ...
    const { data, error } = await admin.rpc("disdagin_create_program", {
    ...
    if (error?.message.includes("DISDAGIN_ACTOR_REQUIRED")) throw new ApiError("Akses ditolak.", 403);
    if (error) throw new ApiError("Program tidak dapat disimpan.", 400);
    await writeActivityLog({ userId: actor.profileId, namaPengguna: actor.namaLengkap, rolePengguna: actor.role, aktivitas: "Menambahkan agenda fasilitasi", modul: "Disdagin", metadata: { programId: data.programId } });
    return NextResponse.json({ ok: true, ...data }, { status: 201 });
  } catch (error) { return apiErrorResponse(error, "Disdagin program creation failed", "Program tidak dapat disimpan."); }
}
