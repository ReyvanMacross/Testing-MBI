...
import { assertInterventionId, parseProgressInput } from "@/lib/disdagin/input";
...
export async function PATCH(request: Request, { params }: { params: Promise<{ interventionId: string }> }) {
  try {
    ...
    const { data, error } = await admin.rpc("disdagin_update_intervention_progress", {
    ...
    await writeActivityLog({
      ...
      aktivitas: "Memperbarui progress fasilitasi akses pasar",
      modul: "Disdagin",
      metadata: { interventionId },
    });
    return NextResponse.json({ ok: true, ...data });
  } catch (error) {
    return apiErrorResponse(error, "Disdagin intervention progress failed", "Progress intervensi tidak dapat disimpan.");
  }
}
