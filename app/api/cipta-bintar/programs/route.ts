...
import { parseProgramInput } from "@/lib/cipta-bintar/input";
...
export async function POST(request: Request) {
  try {
    ...
    const { data, error } = await admin.rpc("cipta_bintar_create_program", {
      p_actor_id: actor.profileId, p_actor_opd_id: actor.opdId, p_code: input.code,
      p_name: input.name, p_category: input.category, p_petugas_id: input.petugasId,
      p_start_date: input.startDate,
      p_duration_value: input.duration, p_duration_unit: input.durationUnit,
      p_capacity: input.capacity, p_budget_per_unit: input.budgetPerUnit, p_description: input.description,
    });
    if (error?.message.includes("PROGRAM_CODE_EXISTS")) throw new ApiError("Kode program sudah digunakan.", 409);
    if (error?.message.includes("CIPTA_BINTAR_ACTOR_REQUIRED")) throw new ApiError("Akses ditolak.", 403);
    if (error) throw new ApiError("Program tidak dapat disimpan.", 400);
    await writeActivityLog({ userId: actor.profileId, namaPengguna: actor.namaLengkap, rolePengguna: actor.role, aktivitas: "Menambahkan program infrastruktur", modul: "CIPTA_BINTAR", metadata: { programId: data.programId } });
    return NextResponse.json({ ok: true, ...data }, { status: 201 });
  } catch (error) { return apiErrorResponse(error, "CiptaBintar program creation failed", "Program tidak dapat disimpan."); }
}
