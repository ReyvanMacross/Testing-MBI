...
import { assertInterventionId, parseCompleteInterventionInput } from "@/lib/cipta-bintar/input";
...
function mapError(error: { message?: string } | null) {
  ...
  if (message.includes("COMPLETION_BEFORE_START")) return new ApiError("Tanggal selesai tidak boleh sebelum petugasan dimulai.", 400);
  return new ApiError("Petugasan tidak dapat diselesaikan.", 400);
}
...
export async function POST(request: Request, { params }: { params: Promise<{ interventionId: string }> }) {
  try {
    ...
    const { data, error } = await admin.rpc("cipta_bintar_complete_intervention", {
      p_intervention_id: interventionId,
      p_actor_id: actor.profileId,
      p_actor_opd_id: actor.opdId,
      p_realization_value: input.realizationValue,
    ...
    await writeActivityLog({
      ...
      aktivitas: "Menyelesaikan rehabilitasi Cipta Bintar",
      modul: "CIPTA_BINTAR",
      metadata: { interventionId, outcomeId: data.outcomeId },
    });
    return NextResponse.json({ ok: true, ...data });
  } catch (error) {
    return apiErrorResponse(error, "Cipta Bintar intervention completion failed", "Rehabilitasi tidak dapat diselesaikan.");
  }
}
