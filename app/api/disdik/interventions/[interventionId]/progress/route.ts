...
import { assertInterventionId, parseProgressInput } from "@/lib/disdik/input";
...
export async function PATCH(request: Request, { params }: { params: Promise<{ interventionId: string }> }) {
  try {
    ...
    const { data, error } = await admin.rpc("disdik_update_intervention_progress", {
    ...
    await writeActivityLog({
      ...
      aktivitas: "Memperbarui progress intervensi pendidikan",
      modul: "Disdik",
      metadata: { interventionId },
    });
    return NextResponse.json({ ok: true, ...data });
  } catch (error) {
    return apiErrorResponse(error, "Disdik intervention progress failed", "Progress intervensi tidak dapat disimpan.");
  }
}
