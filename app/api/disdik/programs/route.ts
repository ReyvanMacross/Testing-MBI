...
import { parseProgramInput } from "@/lib/disdik/input";
...
export async function POST(request: Request) {
  try {
    ...
    const { data, error } = await admin.rpc("disdik_create_program", {
      p_actor_id: actor.profileId, p_actor_opd_id: actor.opdId, p_code: input.code,
      p_name: input.name, p_level: input.level, p_aid_type: input.aidType, p_school_id: input.schoolId,
      p_duration_value: input.duration, p_duration_unit: input.durationUnit,
      p_execution_date: input.executionDate, p_budget_per_student: input.budgetPerStudent,
    ...
    if (error?.message.includes("DISDIK_ACTOR_REQUIRED")) throw new ApiError("Akses ditolak.", 403);
    if (error) throw new ApiError("Program tidak dapat disimpan.", 400);
    await writeActivityLog({ userId: actor.profileId, namaPengguna: actor.namaLengkap, rolePengguna: actor.role, aktivitas: "Menambahkan program pendidikan", modul: "Disdik", metadata: { programId: data.programId } });
    return NextResponse.json({ ok: true, ...data }, { status: 201 });
  } catch (error) { return apiErrorResponse(error, "Disdik program creation failed", "Program tidak dapat disimpan."); }
}
