...
import { assertReferralId, parseStartInterventionInput } from "@/lib/disdik/input";
...
function mapError(error: { message?: string } | null) {
  ...
  if (message.includes("TARGET_OPD_REQUIRED") || message.includes("DISDIK_ACTOR_REQUIRED")) return new ApiError("Akses referral ditolak.", 403);
  if (message.includes("INVALID_PROGRAM")) return new ApiError("Program tidak aktif atau tidak sesuai Disdik.", 400);
  if (message.includes("INVALID_SCHOOL")) return new ApiError("Sekolah tidak aktif atau tidak sesuai program.", 400);
...
export async function POST(request: Request, { params }: { params: Promise<{ referralId: string }> }) {
  try {
    ...
    const { data, error } = await admin.rpc("disdik_start_intervention", {
      ...
      p_student_level: input.studentLevel,
      p_aid_item: input.aidItem,
      p_action_plan: input.actionPlan,
    });
    if (error) throw mapError(error);
    await writeActivityLog({ userId: actor.profileId, namaPengguna: actor.namaLengkap, rolePengguna: actor.role, aktivitas: "Memulai intervensi pendidikan", modul: "Disdik", metadata: { referralId, programId: input.programId } });
    return NextResponse.json({ ok: true, ...data });
  } catch (error) { return apiErrorResponse(error, "Disdik intervention start failed", "Intervensi pendidikan tidak dapat dimulai."); }
}
