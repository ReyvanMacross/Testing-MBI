...
export async function POST(request: Request, { params }: { params: Promise<{ referralId: string }> }) {
  try {
    ...
    const { data, error } = await admin.rpc("disnaker_start_intervention", { p_referral_id: referralId, p_actor_id: actor.profileId, p_actor_opd_id: actor.opdId, p_program_id: input.programId, p_institution: input.institution, p_start_date: input.startDate, p_instruction: input.instruction });
