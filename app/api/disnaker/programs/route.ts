...
import { parseProgramInput } from "@/lib/disnaker/input";
...
export async function POST(request: Request) {
  try {
    ...
    if (error?.message.includes("DISNAKER_ACTOR_REQUIRED")) throw new ApiError("Akses ditolak.", 403);
    if (error) throw new ApiError("Program tidak dapat disimpan.", 400);
    await writeActivityLog({ userId: actor.profileId, namaPengguna: actor.namaLengkap, rolePengguna: actor.role, aktivitas: "Menambahkan program vokasi", modul: "Disnaker", metadata: { programId: data.programId } });
    return NextResponse.json({ ok: true, ...data }, { status: 201 });
  } catch (error) { return apiErrorResponse(error, "Disnaker program creation failed", "Program tidak dapat disimpan."); }
}
