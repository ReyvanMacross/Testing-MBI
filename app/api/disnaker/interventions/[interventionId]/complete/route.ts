...
import { assertInterventionId, parseCompleteInterventionInput } from "@/lib/disnaker/input";
...
function mapError(error: { message?: string } | null) {
  ...
  if (message.includes("INVALID_INDUSTRY_PARTNER")) return new ApiError("Mitra industri tidak valid atau tidak aktif.", 400);
  if (message.includes("PLACEMENT_BEFORE_START")) return new ApiError("Tanggal penempatan tidak boleh sebelum pelatihan dimulai.", 400);
...
export async function POST(request: Request, { params }: { params: Promise<{ interventionId: string }> }) {
  try {
    ...
    const { data, error } = await admin.rpc("disnaker_complete_intervention", {
      p_intervention_id: interventionId,
      p_actor_id: actor.profileId,
      p_actor_opd_id: actor.opdId,
      p_mitra_industri_id: input.mitraIndustriId,
      p_placement_date: input.placementDate,
    ...
    await writeActivityLog({
      ...
      aktivitas: "Menyelesaikan intervensi vokasi",
      modul: "Disnaker",
      metadata: { interventionId, placementId: data.placementId },
    });
    return NextResponse.json({ ok: true, ...data });
  } catch (error) {
    return apiErrorResponse(error, "Disnaker intervention completion failed", "Intervensi tidak dapat diselesaikan.");
  }
}
