...
import { assertInterventionId, parseProgressInput } from "@/lib/diskop/input";
...
function mapError(error: { message?: string } | null) {
  ...
  if (message.includes("TARGET_OPD_REQUIRED") || message.includes("DISKOP_ACTOR_REQUIRED")) return new ApiError("Akses intervensi ditolak.", 403);
...
export async function PATCH(request: Request, { params }: { params: Promise<{ interventionId: string }> }) {
  try {
    ...
    const { data, error } = await admin.rpc("diskop_update_intervention_progress", {
    ...
    await writeActivityLog({
      ...
      aktivitas: "Memperbarui progress pendampingan wirausaha",
      modul: "Diskop UKM",
      metadata: { interventionId },
    });
    return NextResponse.json({ ok: true, ...data });
  } catch (error) {
    return apiErrorResponse(error, "Diskop intervention progress failed", "Progress intervensi tidak dapat disimpan.");
  }
}
