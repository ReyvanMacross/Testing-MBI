...
import { assertInterventionId, parseCompleteInterventionInput } from "@/lib/diskop/input";
...
export async function POST(request: Request, { params }: { params: Promise<{ interventionId: string }> }) {
  try {
    ...
    const { data, error } = await admin.rpc("diskop_complete_intervention", {
    ...
    await writeActivityLog({
      ...
      aktivitas: "Menyelesaikan pendampingan wirausaha",
      modul: "Diskop UKM",
      metadata: { interventionId, outcomeId: data.outcomeId },
    });
    return NextResponse.json({ ok: true, ...data });
  } catch (error) {
    return apiErrorResponse(error, "Diskop intervention completion failed", "Pendampingan tidak dapat diselesaikan.");
  }
}
