...
import { assertInterventionId, parseCompleteInterventionInput } from "@/lib/disbudpar/input";
...
export async function POST(request: Request, { params }: { params: Promise<{ interventionId: string }> }) {
  try {
    ...
    const { data, error } = await admin.rpc("disbudpar_complete_intervention", {
      p_intervention_id: interventionId,
      p_actor_id: actor.profileId,
      p_actor_opd_id: actor.opdId,
      p_achievement_value: input.achievementValue,
    ...
    await writeActivityLog({
      ...
      aktivitas: "Menyelesaikan pendampingan ekraf dan seni",
      modul: "DISBUDPAR",
      metadata: { interventionId, outcomeId: data.outcomeId },
    });
    return NextResponse.json({ ok: true, ...data });
  } catch (error) {
    return apiErrorResponse(error, "Disbudpar intervention completion failed", "Pendampingan tidak dapat diselesaikan.");
  }
}
