...
export type DiskopReferralStatus = "PERLU_DIPROSES" | "SEDANG_DIDAMPINGI" | "MANDIRI_SELESAI";
export type DiskopParticipantStatus = "AKTIF_PENDAMPINGAN" | "MANDIRI_SELESAI" | "TIDAK_AKTIF";
export type DiskopLegalStatus = "BELUM" | "PROSES_NIB_HALAL" | "LEGAL";

export type DiskopTimelineEvent = {
  id: string;
...
export type DiskopReferral = {
  ...
  desil: number;
  kelurahan: string;
  status: DiskopReferralStatus;
  participantStatus: DiskopParticipantStatus | null;
  legalStatus: DiskopLegalStatus | null;
  stimulus: string;
  actionPlan: string;
  progress: number | null;
  nib: string | null;
  ...
  timeline: DiskopTimelineEvent[];
};

export type DiskopProgram = {
  id: string;
  code: string;
  name: string;
  category: string;
...
export const previewEnabled =
  ...
  process.env.DISKOP_PREVIEW_MODE === "true";
...
function mapStatus(status: string): DiskopReferralStatus {
  if (status === "SELESAI") return "MANDIRI_SELESAI";
  if (status === "DITERIMA" || status === "DIPROSES") return "SEDANG_DIDAMPINGI";
  return "PERLU_DIPROSES";
}

export async function getDiskopMentors(): Promise<DiskopMentor[]> {
  if (previewEnabled) return [];
  const admin = createAdminClient();
  const result = await admin
    .from("diskop_pendamping")
    .select("id,kode,nama,klaster,lokasi")
    .eq("is_active", true)
    .order("nama");
  fail(result.error, "Master pendamping Diskop UKM tidak tersedia");
...
export async function getDiskopPrograms(): Promise<DiskopProgram[]> {
  if (previewEnabled) return [];
  const admin = createAdminClient();
  const opd = await admin.from("master_opd").select("id").eq("kode_opd", "DISKOP").single();
  fail(opd.error, "Master OPD DISKOP tidak tersedia");
  if (!opd.data) throw new Error("Master OPD DISKOP tidak tersedia.");
  ...
  const programResult = await admin
    ...
    .eq("jalur", "WIRAUSAHA")
    .order("nama_program");
  fail(programResult.error, "Program Diskop UKM tidak dapat dibaca");
  ...
  fail(detailsResult.error, "Detail program Diskop UKM tidak dapat dibaca");
  fail(interventionsResult.error, "Kapasitas program Diskop UKM tidak dapat dihitung");
  ...
  const mentorResult = mentorIds.length
    ? await admin.from("diskop_pendamping").select("id,nama,klaster,lokasi").in("id", mentorIds)
    : { data: [], error: null };
  fail(mentorResult.error, "Pendamping program Diskop UKM tidak dapat dibaca");
  ...
  return programResult.data.map((program) => {
    ...
    return {
      ...
      facilitation: detail.facilitation ?? "Pendampingan legalitas dan pengembangan usaha",
    };
  });
}

export async function getDiskopReferrals(): Promise<DiskopReferral[]> {
  if (previewEnabled) return [];
  const admin = createAdminClient();
  const opd = await admin.from("master_opd").select("id").eq("kode_opd", "DISKOP").single();
  fail(opd.error, "Master OPD DISKOP tidak tersedia");
  ...
  const referralResult = await admin
    ...
    .limit(100);
  fail(referralResult.error, "Referral Diskop UKM tidak dapat dibaca");
  ...
  fail(interventionResult.error, "Intervensi Diskop UKM tidak dapat dibaca");
  ...
  const [wargaResult, desilResult, programsResult, detailsResult, eventsResult, outcomesResult] = await Promise.all([
    ...
    programIds.length ? admin.from("diskop_program_details").select("program_id,pendamping_id,consultant").in("program_id", programIds) : Promise.resolve({ data: [], error: null }),
    interventionIds.length ? admin.from("diskop_intervention_events").select("id,intervention_id,event_type,event_at,note,progress_percent").in("intervention_id", interventionIds).order("event_at", { ascending: true }) : Promise.resolve({ data: [], error: null }),
    interventionIds.length ? admin.from("diskop_kemandirian_usaha").select("intervention_id,nib,omzet_bulanan,evaluasi_akhir").in("intervention_id", interventionIds) : Promise.resolve({ data: [], error: null }),
  ]);
  for (const [result, context] of [
    [wargaResult, "Warga referral Diskop UKM tidak dapat dibaca"],
    [desilResult, "Desil referral Diskop UKM tidak dapat dibaca"],
    [programsResult, "Program referral Diskop UKM tidak dapat dibaca"],
    [detailsResult, "Detail program referral Diskop UKM tidak dapat dibaca"],
    [eventsResult, "Timeline pendampingan Diskop UKM tidak dapat dibaca"],
    [outcomesResult, "Kemandirian usaha Diskop UKM tidak dapat dibaca"],
  ...
  const eventMap = new Map<string, DiskopTimelineEvent[]>();
  ...
  return referrals.flatMap((referral): DiskopReferral[] => {
...
export async function getDiskopDashboardData() {
  const referrals = await getDiskopReferrals();
  return {
    referrals,
    summary: {
...
export async function getDiskopRevenueReport() {
  ...
  fail(interventionResult.error, "Intervensi laporan Diskop UKM tidak dapat dibaca");
  const interventions = interventionResult.data ?? [];
  ...
  const [referralResult, historyResult] = await Promise.all([
    ...
    admin.from("diskop_laporan_omzet").select("id,intervention_id,periode,nominal,status").in("intervention_id", interventionIds).order("periode", { ascending: false }),
  ]);
  fail(referralResult.error, "Referral laporan Diskop UKM tidak dapat dibaca");
  fail(historyResult.error, "Riwayat omzet Diskop UKM tidak dapat dibaca");
  const wargaIds = [...new Set((referralResult.data ?? []).map((row) => row.warga_id))];
  const [wargaResult, desilResult] = await Promise.all([
    admin.from("warga").select("id,nik,nama_lengkap,kelurahan").in("id", wargaIds),
    admin.from("penetapan_desil").select("warga_id,desil_dtsen,created_at").in("warga_id", wargaIds).order("created_at", { ascending: false }),
  ]);
  fail(wargaResult.error, "Warga laporan Diskop UKM tidak dapat dibaca");
  fail(desilResult.error, "Desil laporan Diskop UKM tidak dapat dibaca");
