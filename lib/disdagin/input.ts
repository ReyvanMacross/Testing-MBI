...
export function parseStartInterventionInput(body: unknown) {
  ...
  if (typeof input.pendampingId !== "string" || !UUID.test(input.pendampingId)) {
    throw new ApiError("Mitra atau penyelenggara wajib dipilih.", 400);
  }
  return {
    programId: input.programId,
    pendampingId: input.pendampingId,
    businessName: text(input.businessName, "Nama usaha", 3, 200),
    businessCategory: text(input.businessCategory, "Kategori usaha", 2, 100),
    nib: (() => {
      const value = typeof input.nib === "string" ? input.nib.replace(/\D/g, "") : "";
      if (!NIB.test(value)) throw new ApiError("NIB harus tepat 13 digit.", 400);
      return value;
    })(),
...
export function parseProgressInput(body: unknown) {
  ...
  if (!["BELUM", "PROSES_NIB_HALAL", "LEGAL"].includes(legalStatus)) {
    throw new ApiError("Status penetrasi pasar tidak valid.", 400);
  }
  return {
    participantStatus,
    progressPercent,
...
export function parseProgramInput(body: unknown) {
  ...
  if (!/^PRG-[A-Z]{3}-[0-9]{2}$/.test(code)) {
    throw new ApiError("Kode program harus memakai format PRG-DAG-01.", 400);
  }
  if (typeof input.pendampingId !== "string" || !UUID.test(input.pendampingId)) {
    throw new ApiError("Mitra atau penyelenggara wajib dipilih.", 400);
  }
