...
begin
  ...
  elsif tg_table_name in ('disbudpar_kemandirian_ekraf', 'disbudpar_laporan_pembinaan', 'disbudpar_intervention_events') then
  ...
  if not exists (
    select 1 from public.disbudpar_intervention_events
  ...
  if outcome_row.id is not null
     and (intervention_row.participant_status <> 'MANDIRI_SELESAI'
          ...
          or intervention_row.creative_result_status <> 'AKTIF_TAMPIL_PRODUKSI_RUTIN'
  ...
  if outcome_row.id is not null
     and not exists (
       select 1 from public.disbudpar_laporan_pembinaan
...
drop trigger if exists trg_disbudpar_integrity_report on public.disbudpar_laporan_pembinaan;
create constraint trigger trg_disbudpar_integrity_report
after insert or update or delete on public.disbudpar_laporan_pembinaan
deferrable initially deferred for each row execute function public.disbudpar_validate_domain_integrity();

drop trigger if exists trg_disbudpar_integrity_event on public.disbudpar_intervention_events;
create constraint trigger trg_disbudpar_integrity_event
after insert or update or delete on public.disbudpar_intervention_events
deferrable initially deferred for each row execute function public.disbudpar_validate_domain_integrity();
