...
begin
  ...
  insert into public.disdik_realisasi_bantuan(intervention_id,aid_item,realized_amount,disbursement_date,notes,created_by,updated_by) values(p_intervention_id,i.aid_item,p_realized_amount,(clock_timestamp() at time zone 'Asia/Jakarta')::date,trim(p_evaluation_note),p_actor_id,p_actor_id) on conflict(intervention_id) do update set realized_amount=excluded.realized_amount,disbursement_date=excluded.disbursement_date,notes=excluded.notes,updated_by=p_actor_id,updated_at=now();
