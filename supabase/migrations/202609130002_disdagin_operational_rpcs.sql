-- Transactional operations for the Disdagin WIRAUSAHA lifecycle.

create or replace function public.disdagin_actor_allowed(p_actor_id uuid, p_opd_id uuid)
returns boolean
language sql
stable
security definer
...
as $$
  select exists (
    ...
    where profile.id = p_actor_id
      ...
      and opd.kode_opd = 'DISDAGIN'
  );
$$;

create or replace function public.disdagin_start_intervention(
  p_referral_id uuid,
  p_actor_id uuid,
  p_actor_opd_id uuid,
  p_program_id uuid,
...
declare
  ...
  mentor_row public.disdagin_pendamping%rowtype;
  active_participants bigint;
  new_intervention_id uuid;
  business_profile_id uuid;
begin
  if not public.disdagin_actor_allowed(p_actor_id, p_actor_opd_id) then
    raise exception using errcode = '42501', message = 'DISDAGIN_ACTOR_REQUIRED';
  end if;
  if p_referral_id is null
     ...
     or length(trim(coalesce(p_business_name, ''))) not between 3 and 200
     or length(trim(coalesce(p_business_category, ''))) not between 2 and 100
     or p_nib is null
     or p_nib !~ '^[0-9]{13}$'
  ...
  if referral_row.status <> 'TERKIRIM'
     or exists (select 1 from public.disdagin_interventions where referral_id = p_referral_id) then
  ...
  select * into details_row from public.disdagin_program_details where program_id = p_program_id for update;
  ...
  select * into mentor_row from public.disdagin_pendamping where id = p_pendamping_id and is_active;
  ...
  from public.disdagin_interventions
  ...
  insert into public.disdagin_business_profiles (
    warga_id, nama_usaha, kategori_usaha, nib, is_fixture, created_by, updated_by
  ) values (
    referral_row.warga_id, trim(p_business_name), trim(p_business_category), p_nib,
    referral_row.is_fixture, p_actor_id, p_actor_id
  )
  on conflict (warga_id) do update set
    nama_usaha = excluded.nama_usaha,
    kategori_usaha = excluded.kategori_usaha,
    nib = excluded.nib,
    is_fixture = excluded.is_fixture,
    updated_by = excluded.updated_by
  returning id into business_profile_id;

  insert into public.disdagin_interventions (
    referral_id, program_id, pendamping_id, business_profile_id, consultant, start_date,
    stimulus, action_plan, created_by, updated_by
  ) values (
    p_referral_id, p_program_id, p_pendamping_id, business_profile_id, mentor_row.nama, p_start_date,
    trim(p_stimulus), trim(p_action_plan), p_actor_id, p_actor_id
  ) returning id into new_intervention_id;

  perform public.transition_referral_status(p_referral_id, 'DITERIMA', p_actor_id, p_actor_opd_id, 'Rujukan diterima untuk fasilitasi akses pasar.');
  perform public.transition_referral_status(p_referral_id, 'DIPROSES', p_actor_id, p_actor_opd_id, 'Peserta mulai mengikuti agenda fasilitasi wirausaha.');

  insert into public.disdagin_intervention_events (
    intervention_id, event_type, progress_percent, legal_status, note, actor_user_id
  ) values (
    new_intervention_id, 'STARTED', 0, 'BELUM', 'Program fasilitasi akses pasar dimulai.', p_actor_id
  );
...
create or replace function public.disdagin_update_intervention_progress(
...
begin
  ...
  select * into intervention_row from public.disdagin_interventions where id = p_intervention_id for update;
  ...
  update public.disdagin_interventions set
...
create or replace function public.disdagin_complete_intervention(
  p_intervention_id uuid,
  p_actor_id uuid,
  p_actor_opd_id uuid,
  p_nib text,
...
begin
  ...
  if referral_row.status <> 'DIPROSES'
     ...
     or exists (select 1 from public.disdagin_kemandirian_usaha where intervention_id = p_intervention_id) then
    raise exception using errcode = '23505', message = 'INTERVENTION_ALREADY_COMPLETED';
  end if;
  ...
  if program_row.id is null or details_row.program_id is null or business_row.id is null then
    raise exception using errcode = '23514', message = 'INVALID_PROGRAM';
  end if;
  if business_row.warga_id <> referral_row.warga_id or business_row.nib <> p_nib then
    raise exception using errcode = '23514', message = 'INVALID_BUSINESS_PROFILE';
  end if;
...
exception
  when unique_violation then
    if exists (select 1 from public.disdagin_kemandirian_usaha where nib = p_nib and intervention_id <> p_intervention_id) then
...
create or replace function public.disdagin_create_program(
  p_actor_id uuid,
  p_actor_opd_id uuid,
  p_code text,
  p_name text,
...
begin
  ...
  insert into public.disdagin_program_details (
...
revoke all on function public.disdagin_actor_allowed(uuid, uuid) from public, anon, authenticated;
grant execute on function public.disdagin_actor_allowed(uuid, uuid) to service_role;
revoke all on function public.disdagin_start_intervention(uuid, uuid, uuid, uuid, uuid, text, text, text, date, text, text) from public, anon, authenticated;
grant execute on function public.disdagin_start_intervention(uuid, uuid, uuid, uuid, uuid, text, text, text, date, text, text) to service_role;
revoke all on function public.disdagin_update_intervention_progress(uuid, uuid, uuid, text, integer, text, text) from public, anon, authenticated;
grant execute on function public.disdagin_update_intervention_progress(uuid, uuid, uuid, text, integer, text, text) to service_role;
revoke all on function public.disdagin_complete_intervention(uuid, uuid, uuid, text, bigint, date, text) from public, anon, authenticated;
grant execute on function public.disdagin_complete_intervention(uuid, uuid, uuid, text, bigint, date, text) to service_role;
revoke all on function public.disdagin_create_program(uuid, uuid, text, text, text, uuid, integer, text, integer, text) from public, anon, authenticated;
grant execute on function public.disdagin_create_program(uuid, uuid, text, text, text, uuid, integer, text, integer, text) to service_role;
