-- Final Dkpp integrity hardening. Cross-table state is validated at commit.

create unique index if not exists uq_dkpp_single_lifecycle_event
on public.dkpp_intervention_events(intervention_id, event_type)
...
begin
  ...
  elsif tg_table_name = 'dkpp_ketahanan_pangan' then
  ...
  if outcome_row.id is not null
     and (intervention_row.participant_status <> 'MANDIRI_SELESAI'
          ...
          or intervention_row.harvest_status <> 'MEMENUHI_DAN_DIPASARKAN') then
...
drop trigger if exists trg_dkpp_integrity_intervention on public.dkpp_interventions;
create constraint trigger trg_dkpp_integrity_intervention
after insert or update or delete on public.dkpp_interventions
deferrable initially deferred for each row execute function public.dkpp_validate_domain_integrity();

drop trigger if exists trg_dkpp_integrity_food_outcome on public.dkpp_ketahanan_pangan;
create constraint trigger trg_dkpp_integrity_food_outcome
after insert or update or delete on public.dkpp_ketahanan_pangan
deferrable initially deferred for each row execute function public.dkpp_validate_domain_integrity();

drop trigger if exists trg_dkpp_integrity_beneficiary_profile on public.dkpp_beneficiary_profiles;
create constraint trigger trg_dkpp_integrity_beneficiary_profile
after insert or update or delete on public.dkpp_beneficiary_profiles
deferrable initially deferred for each row execute function public.dkpp_validate_domain_integrity();

drop trigger if exists trg_dkpp_integrity_program_details on public.dkpp_program_details;
create constraint trigger trg_dkpp_integrity_program_details
after insert or update or delete on public.dkpp_program_details
deferrable initially deferred for each row execute function public.dkpp_validate_domain_integrity();

drop trigger if exists trg_dkpp_integrity_master_program on public.master_program_layanan;
create constraint trigger trg_dkpp_integrity_master_program
after insert or update or delete on public.master_program_layanan
deferrable initially deferred for each row execute function public.dkpp_validate_domain_integrity();

drop trigger if exists trg_dkpp_integrity_referral on public.referral_mbi;
create constraint trigger trg_dkpp_integrity_referral
